function record(chatterPlayer, arguments, bot) {
    ...
    if (arguments == null || arguments == "") {
        ...
        bot.sendGameMessage('Utilisation: $record on OU $cr off')
    }
        ...
        bot.sendGameMessage('Vous avez fourni trop de paramètres. Utilisez la commande "$record" ou "$cr" pour mieux comprendre son utilisation')
    }
    else if (arguments.toLowerCase() != "on" && arguments.toLowerCase() != "off") {
        bot.sendGameMessage('Le paramètre renseigné doit avoir pour valeur "on" ou "off". Utilisez la commande "$record" ou "$cr" pour mieux comprendre son utilisation')
    }
    else {
        if (arguments.toLowerCase() == "on" && !bot.get_isRanked()) {
            ...
            bot.sendGameMessage('Début du mode classement')
            ...
            bot.sendGameMessage('Fin du mode classement')
        }  
        else if (arguments.toLowerCase() == "off" && !bot.get_isRanked()) {
            bot.sendGameMessage("Le mode classement n'est pas actif")
        }  
        else if (arguments.toLowerCase() == "on" && bot.get_isRanked()) {
            bot.sendGameMessage("Le mode classement est déjà actif")
        }  
    }
}
