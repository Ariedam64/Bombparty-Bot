async function setUserModerator(chatterPlayer, arguments, bot) {
    ...
    if (chatterPlayer.auth == null || !bot.get_playerStaff().includes(chatterPlayer.auth.id) && chatterPlayer.auth.id != bot.get_creatorId()) {
        ...
        bot.sendGameMessage('Cette commande permet au bot de mettre un joueur modérateur. La commande prend en paramètre le pseudo (jklm, twitch ou discord) ou le peerId du joueur')
        bot.sendGameMessage('Utilisation: $mod Ayaya OU $rm Ayaya')
    }
        ...
        if (player != null) {
            bot.get_wsRoom().emitCustom(5, "setUserModerator", player.get_peerId(), true)
        }
    }
}

module.exports = setUserModerator