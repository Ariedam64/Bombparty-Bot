async function setLeader(chatterPlayer, arguments, bot) {
    ...
    if (chatterPlayer.auth == null || !bot.get_playerStaff().includes(chatterPlayer.auth.id) && chatterPlayer.auth.id != bot.get_creatorId()) {
        ...
        bot.sendGameMessage('Cette commande permet au bot de mettre un joueur leader du salon. La commande prend en paramètre le pseudo (jklm, twitch ou discord) ou le peerId du joueur')
        bot.sendGameMessage('Utilisation: $lead Ayaya OU $rl Ayaya')
    }
        ...
        if (player != null) {
            bot.get_wsRoom().emitCustom(5, "setUserLeader", player.get_peerId())   
        }
    }
}

module.exports = setLeader