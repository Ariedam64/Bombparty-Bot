async function unsetUserModerator(chatterPlayer, arguments, bot) {
    ...
    if (chatterPlayer.auth == null || !bot.get_playerStaff().includes(chatterPlayer.auth.id) && chatterPlayer.auth.id != bot.get_creatorId()) {
        ...
        bot.sendGameMessage('Cette commande permet au bot d\'enlever le rôle modérateur à un joueur. La commande prend en paramètre le pseudo (jklm, twitch ou discord) ou le peerId du joueur')
        bot.sendGameMessage('Utilisation: $unmod Ayaya OU $rum Ayaya')
    }
...
module.exports = unsetUserModerator