async function unsetUserBanned(chatterPlayer, arguments, bot) {
    ...
    if (chatterPlayer.auth == null || !bot.get_playerStaff().includes(chatterPlayer.auth.id) && chatterPlayer.auth.id != bot.get_creatorId()) {
        ...
        bot.sendGameMessage('Cette commande permet au bot de débannir un joueur. La commande prend en paramètre le pseudo (jklm, twitch ou discord) ou le peerId du joueur')
        bot.sendGameMessage('Utilisation: $unban Ayaya OU $rub Ayaya')
    }
...
module.exports = unsetUserBanned